...
Based on a U-Net with convolution layers, downsampling and upsampling stages, skip connections, residual learning concepts.

Mathematically, the model learns:
\begin{equation}
  f_\Theta​(\bm{X}+\bm{N}_0)\approx \rm{X}+\bm{N}_1
\end{equation}
where $\bm{X}$is the clean signal, and $\bm{N}_0$, and $\bm{N}_2$ are independent noise samples.

The standard loss is
\begin{equation}
  L = \mathbb{E}[||f_\Theta​(\bm{\tilde{X}^{(0)}})-\bm{\tilde{X}^{(1)}}||^2],
\end{equation}
where $\bm{\tilde{X}^{(0)}}$ and $\bm{\tilde{X}^{(1)}}$ are two noisy observations.
...
% J. Lehtinen, “Noise2noise: Learning image restoration without clean data,” arXiv preprint arXiv:1803.04189, 2018.
 ...
 ZeroShotN2N [14] utilizes checkerboard subsampling to fit a light-weight network to a single noisy image

 [14] oussef Mansour and Reinhard Heckel. “Zero-Shot Noise2Noise: Efficient Image Denoising without any
 Data”. In: Proceedings of the IEEE Computer Society Conference on Computer Vision and Pattern Recognition. Vol. 2023-June. 2023. DOI: 10.1109/CVPR52729.2023.01347.
