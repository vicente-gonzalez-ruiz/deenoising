...
In digital signals, \emph{Noise} refers to any unwanted (and usually
unpredectible\footnote{Structured noise, for example, can be spatially
  modeled and therefore, predectible.}) alteration of the original
signal, which can corrupt the information being transmitted or
processed. In other words, noise is any interference that distorts a
\emph{clean} signal, generating a \emph{noisy} signal.
...
\begin{comment}
However, because we are interested in denoising
...
\section{Stochastic VS structured noise}
Noise can be stochastic (\gls{idd}), such as Poisson or Gaussian, or
structured (correlated).  Structured corruptions exhibit correlations
along dominant orientations and patterns that are coherent spatially,
temporally, or otherwise. \cite{wang2025median2median} % https://arxiv.org/pdf/2510.01666
...
\section{\glsentryfull{AWG} noise}
\label{sec:AWG}
%{{{
...
Zero-mean \footnote{In most of references, this particularity is
  ...
  noisy signal the expectation of the noisy signal.} \acrshort{AWG} noise
...
\item \textbf{Additive}: The noise is added to the signal:
  ...
  that follow a Gaussian (normal) distribution\footnote{Read and dark
  noise of most sensors can be treated as additive Gaussian noise.}
  (see Section~\ref{sec:gaussian_distribution}), centered at zero:
  \begin{equation} 
...
For these reasons, this type of noise is commonly refered as \gls{AWG} noise.
...
\begin{figure}
  ...
  \resizebox{1.0\textwidth}{!}{
    ...
    \begin{tabular}{ccc}
      \href{https://nbviewer.org/github/vicente-gonzalez-ruiz/denoising/blob/main/notebooks/2d_gaussian.ipynb}{\includegraphics{notebooks/2d_gaussian.pdf}} & \href{https://nbviewer.org/github/vicente-gonzalez-ruiz/denoising/blob/main/notebooks/2d_gaussian_AWG_20.ipynb}{\includegraphics{2d_gaussian_AWG_20.pdf}} & \href{https://nbviewer.org/github/vicente-gonzalez-ruiz/denoising/blob/main/notebooks/2d_gaussian_AWG_100.ipynb}{\includegraphics{2d_gaussian_AWG_100.pdf}}
    \end{tabular}
  }
...
% The Cause: Light travels in discrete packets of energy called photons. These photons do not arrive at the sensor in a perfectly steady stream; they arrive randomly. The Analogy: Imagine leaving a grid of buckets (your camera sensor) out in a steady rainstorm. Even if the rain is completely uniform, at any given second, one bucket might catch 10 raindrops while the bucket next to it catches 12. That random statistical variation is photon noise. Photon noise follows Poisson statistics. The noise is equal to the square root of the number of photons captured: Photon Noise=sqrt(Signal). Because the noise grows as the square root of the signal, the signal-to-noise ratio (SNR) actually improves as you collect more light. A brighter image has more total photon noise, but the noise becomes much less noticeable compared to the overwhelming amount of signal. How to fix it: You cannot eliminate it. The only way to reduce the relative impact of photon noise is to capture more light (use a wider aperture, longer exposure, or more lighting). (Gemini)

% Dark noise is a hardware imperfection. It is generated by the sensor itself, even if you put a lens cap on the camera and shoot in pitch blackness. The Cause: Camera sensors are made of silicon. As the silicon heats up, thermal energy occasionally knocks an electron loose within the sensor. The sensor's electronics misinterpret this wandering "thermal electron" as a real photon of light. The Analogy: Returning to the bucket analogy, dark noise is like having tiny, unpredictable leaks at the bottom of the buckets that occasionally push groundwater up into the bucket, adding fake "rain" to your measurement. The Math: Like photon noise, dark noise also follows Poisson statistics based on the dark current. Dark Noise=sqrt(Dark Current×Exposure Time). How it behaves: Dark noise increases primarily with two factors: temperature and exposure time. The hotter the sensor gets, and the longer it is left on to capture an image, the more thermal electrons are generated. How to fix it: Because it is hardware-dependent, you can actively reduce it. Professional astrophotography and scientific cameras use active cooling systems (like liquid nitrogen or Peltier coolers) to freeze the sensor, which practically stops dark current dead in its tracks. You can also mathematically subtract a "dark frame" (a picture taken with the lens cap on at the same temperature and exposure time) from your final image.
...
relevant in low-radiation microscopy imaging. Shot noise is typically
modeled as Poission noise.
...
\subsection{\acrshort{SNR} of the Poisson noise model}
...
\subsection{Simulation using Gaussian noise generators}
Considering Eq.~\ref{eq:E_Poisson} and Eq.~\ref{eq:V_Poisson}, it should be clear that we can simulate the generation of a Poisson sequence using
\begin{equation}
  \hat{\mathbf{s}}_i \approx \mathbf{s}_i + \mathbf{n}_i^{\mathcal{N}(\mu=\mathbf{s}_i, \sigma=\mathbf{s}_i/\gamma)}.
  \label{eq:Poisson_simulation}
\end{equation}
...
\begin{figure}
  ...
  \resizebox{1.0\textwidth}{!}{
    ...
    \setlength{\tabcolsep}{0ex}      % Adjust column spacing in the table    
    \begin{tabular}{ccc}
      \href{https://nbviewer.org/github/vicente-gonzalez-ruiz/denoising/blob/main/notebooks/2d_gaussian.ipynb}{\includegraphics{2d_gaussian.pdf}} & \href{https://nbviewer.org/github/vicente-gonzalez-ruiz/denoising/blob/main/notebooks/Poisson_2d_gaussian.ipynb}{\includegraphics{Poisson_2d_gaussian_0.01.pdf}} & \href{https://nbviewer.org/github/vicente-gonzalez-ruiz/denoising/blob/main/notebooks/Poisson_2d_gaussian.ipynb}{\includegraphics{notebooks/Poisson_2d_gaussian_0.1.pdf}}
    \end{tabular}
  }
...
\section{\glsentryfull{MPG} noise}
\label{sec:MPG}
% {{{
...
\acrshort{MPG} noise \cite{meiniel2018denoising}. The generation of
...
\subsection{\acrshort{SNR} of the \acrshort{MPG} noise model}
...
\gls{MPG} noise can be generated with
\begin{equation}
  \hat{\mathbf{s}}_i \approx \mathbf{s}_i + \mathbf{n}_i^{\mathcal{N}(\mu=\mathbf{s}_i, \sigma= (1-\alpha)^2\sigma^2 + \alpha^2\mathbf{s}_i/\gamma)}.
  \label{eq:Poisson_simulation}
\end{equation}
...
A significative number of denoising techniques (see for example the
Chapter~\ref{cha:WF}) require to know the variance of the noise to
select their working parameters.
...
This information can be obtained using the \gls{FSC} curve that
measures the similarity between two 3D volumes represented in the
Fourier domain \cite{harauz1986exact,verbeke2024self} (for the 2D
case, the equivalent metric is \gls{FRC}
\cite{van1982arthropod}). Each point of the curve prepresents the
normalized cross-correlation (see Section \ref{sec:cross-correlation})
between two ``shells'' (``rings'' in 2D) of Fourier coefficients of
both volumes (images). Correlation in the Fourier domain has become
...
\section{\glsentryfull{SFC} curve}
%{{{
...
\section{Estimation of the variance of the noise using the \gls{SFC} curve}
\label{sec:variance_SFC}

\begin{subappendices}

\section{\glsentryfull{EOS}}
\label{sec:EOS}
%{{{

In the 2D case, \acrshort{EOS} generates
\begin{equation}
...
\section{\glsentryfull{ICBS}}
\label{sec:ICBS}
%{{{

\acrshort{ICBS} is an extension of \gls{CBS} (which must be run first, see
...
\section{\glsentryfull{SCBS}}
\label{sec:SCBS}
%{{{

\acrshort{SCBS} \cite{koho2019fourier} decomposes an input image $\mathbf{X}$
...
\section{\glsentryfull{TRPR}}
\label{sec:TRPR}
%{{{

\acrshort{TRPR} divides one noisy image into overlaping $2\times 2$ patches
...
\section{\glsentryfull{SPRS}}
\label{sec:SPRS}
%{{{

\acrshort{SPRS} can be used to generate similar instances of an image or volume
