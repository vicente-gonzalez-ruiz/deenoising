...
\section{Discrete randon vector} 

A \gls{DRVect} expands the concept of a random variable by
mapping outcomes to a point in multi-dimensional space rather than a
single number. Eq.~\cite{eq:DRV} holds, but now $\mathcal{X}$ is a
countable subset of $n$-dimensional space.

\begin{equation}
\mathbf{X}(\omega) = [ X_1(\omega), X_2(\omega), \cdots, X_n(\omega)]^\text{T}
\end{equation}

The use of \glspl{DRVect} allows us to bundle multiple characteristics
