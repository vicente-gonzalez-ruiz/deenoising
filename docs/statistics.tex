...
will denote the sample space with $\mathbf{\Omega}$, a sample as
$\mathbf{\omega}$, the \gls{DRVec} as $\mathbf{X}$, and the
realization space as $\mathcal{X}^N$.
...
\item For example, suppose we roll two ($N=2$) fair six-sided
  ...
  \gls{DRVec} $\mathbf{X}=[S,D]^{\text{T}}$, where $S=d_1 + d_2$ and
  $D=|d_1-d_2|$ are \glspl{DRV}. For example, if
  $\boldsymbol{\omega}=[4,6]^{\text{T}}$, we get
  $\mathbf{X}(\boldsymbol{\omega})=[10,2]^{\text{T}}$.
