...
\section{\gls{DRV}} % Discrete Random Variable
...
\begin{equation}
\mathbf{X}(\omega) = [\mathbf{X}_1(\omega), \mathbf{X}_2(\omega), \cdots, \mathbf{X}_n(\omega)]^\text{T}.
\end{equation}
...
$d_i\in\{1,\cdots,6\}$. We define a \gls{DRVec} $\mathbf{X}=(S,D)$
where $S$ is the sum of the dice ($d_1 + d_2$) and $D$ is the
absolute difference ($|d_1-d_2|$). Now, we define our random vector as $\mathbf{X}=[X_1,X_2]^{\text{T}}$

If the outcome is $\omega=(4,6)$, the random vector maps this to:
\begin{equation}
\mathbf{X}(4, 6) = \begin{bmatrix} 10 \ 2 \end{bmatrix}
\end{equation}
