...
\begin{equation}
  ...
  \label{eq:DRV}
\end{equation}
that maps outcomes $\omega\in\Omega$ (the samples space) to numerical
values in some set $\mathcal{X}$, usually $\mathbb{N}^+$ or a
countable subset. For example, if we toss a coin, $X$ is a
random variable that numerically represents the coin flip,
