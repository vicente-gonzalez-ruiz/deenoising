...
statistically using a \gls{DRVec}.\footnote{By physical requirements,
  digital signals are finite in length.} We can identify:
\begin{enumerate}
\item $\Omega$ (the sample space) is the set of all possible physical
  ...
  sampling window, and $\omega$ (the ``analog reality'', i.e., the
  signal to digitalize) is a single, specific continuous voltage
  waveform that actually occurs during your measurement
  window.
\item $X$ represents the \gls{ADC}. It is a deterministic function
  that performs two operations on the outcome $\omega$:
  \begin{enumerate}
  \item Sampling: $X$ looks at $\omega$ at $N$ specific time points
    $\{t_1,t_2,\cdots,t_N\}$.
  \item Quantization: Then $X$ usually rounds the voltage at those
...
The expected value, expectation or expectancy of a random variable
$X$, denoted by $\mathbb{E}[X]$, is its the long-term average
outcome. It is defined as the center of mass of the probability
distribution. Specifically, for a \gls{DRV},
\begin{equation}
...
\begin{equation}
  \mu(x) = \sum_\Omega X(\omega)d\Pr(\omega),
  \label{eq:sample_mean}
\end{equation}
i.e. (see Eq.~\ref{eq:expectation} and Eq.~{eq:moments}), is the
expected value of the \gls{DRV}.

\section{\gls{LLN}}
The \gls{LLN} expresses the fact that our empirical observations will
eventually align with our theoretical models if we collect enough
data. Let $X^{(1)}, X^{(2)}, \cdots, X^{(N)}$.

\section{Average}
\label{sec:average}
The average of the realizations $\{x^{(i)}\}_{i=1}^{N}$ generated after $N$
iterations of a random variable is defined by
\begin{equation}
  \overline{x} = \frac{1}{N}\sum_{i=1}^{N}x^{(i)}.
  \label{eq:average}
\end{equation}

Notice that by the law of large numbers, as the number of realizations
$N\rightarrow\infty$, the average $\overline{x}$ convertes to the
sample mean $\mu(x)$.
