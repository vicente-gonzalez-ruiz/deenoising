...
$X$ is a function that maps an \emph{outcome} $\omega\in\Omega$ to a
\emph{realization} $x\in\mathcal{X}$:
\begin{equation}
...
By physical requirements, digital signals are finite in length. The
digitalization of a sequence of $N$ samples can be modeled
statistically using a \gls{DRVec}. We can recognize that:
\begin{itemize}
...
The \gls{PMF} of a random variable (or vector) $X$ is defined as
\begin{equation}
