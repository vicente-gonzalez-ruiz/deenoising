...
event occurs in a fixed interval of event domain (usually time or
space), under the following assumptions:
\begin{enumerate}
...
Its \gls{PMF} is:
\begin{equation}
  \Pr(X{=}\bm{x}_i) = \frac{e^{-\lambda}\lambda^{\bm{x}_i}}{\bm{x}_i!},
  \label{eq:PN}
\end{equation}
