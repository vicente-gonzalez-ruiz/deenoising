...
\section{Expectation of a \gls{DRV}}

The expected value, expectation or expectancy of a random variable $X$, denoted by $\mathbb{E}[X]$, is its the long-term average outcome. It is defined as the center of mass of the probability distribution. Specifically, for a \gls{DRV},
...
\label{sec:mean}
The sample mean of a \gls{DRV} $X$, usually denoted by $\overline{x}$, is the
first moment of the realizations generated by a number of iterations
of the \gls{DRV}. Therefore,
\begin{equation}
  \overline{x} = \frac{1}{N}\sum_{i=1}^{N}x_i,
  \label{eq:mean}
\end{equation}
where $N$ is the number of iterations and $x_i$ if the $i$-th realization. However, notice that we will obtain the same result if we compute
\begin{equation}
  \overline{x} = \sum_\Omega X(\omega)dP(\omega),
  \label{eq:mean2}
\end{equation}
which is a definition based on the sample space.
