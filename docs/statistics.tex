...
\section{\glsentryfullpl{DRV}}

In general, a \gls{DRV} $X$ is a function
\begin{equation}
  X:\Omega\rightarrow \mathcal{X}
  \label{eq:DRV}
\end{equation}
that maps outcomes a single random value $\omega\in\Omega$ (the sample
space) to a numerical value in some set $\mathcal{X}$, usually
$\mathbb{N}$ or a countable subset. For example, if we toss a coin,
$X$ is a random variable that numerically represents a coin flip
$\omega$ (in our example,
$\Omega=\{\text{``Heads''},\text{``Tails''}\}$), and for example, if
were $\omega=\text{``Heads''}$ then $X(\omega)=0$, and viceversa,
$X(\omega)=1$ if $\omega=\text{``Tails''}$.

Notice that, in general, a randon variable $X$ is not random itself;
it's a deterministic function of the underlying outcome (for example,
$X(\text{Heads})=0$, always). The ``randomness'' comes from the fact
that the outcome $\omega$ is random (governed by the probability of
resulting heads or tails).

\section{\glsentryfull{DRVec}} 

By definition, a \gls{DRV} $X$ can only model the transformation of a
single $w\in\Omega$, that is the same, can only model the generation
of a single $x\in\mathcal{X}$ (in the context of digital signals, a
\gls{DRV} would only handle a single sample). To deal with a complete
digital signal (or sequence of samples) we need to extend the concept
of \gls{DRV} to finite sequences (of possiblely correlated)
\glspl{DRV}.

A \gls{DRVec} $\mathbf{X}$ maps outcomes to a point in
multi-dimensional space rather than a single number. Now
Eq.~\ref{eq:DRV} becomes
\begin{equation}
  \mathbf{X}:\Omega\rightarrow \mathcal{X},
  \label{eq:DRVec}
\end{equation}
where $\mathcal{X}$ is a countable subset of a $n$-dimensional space, and 
\begin{equation}
\mathbf{X} = [\mathbf{X}_1, \mathbf{X}_2, \cdots, \mathbf{X}_n]^\text{T}.
\end{equation}

The use of \glspl{DRVec} allows us to bundle multiple characteristics
of a single experiment into a single mathematical object. For example,
suppose we roll two fair six-sided dice. The sample space $\Omega$
consists of 36 ordered pairs $(d_1,d_2)$ where
$d_i\in\{1,\cdots,6\}$. Now, we define a \gls{DRVec}
$\mathbf{X}=[S,D]^{\text{T}}$ where $S=d_1 + d_2$ and $D=|d_1-d_2|$
are discrete random variables. For example, if the outcome is
$\omega=(4,6)$, the random vector maps this to
$\mathbf{X}(4,6)=[10,2]^{\text{T}}$.
...
and the existence of the terms covariance and correlation.
