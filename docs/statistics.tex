...
\begin{equation}
  ...
  \label{eq:DRV}
\end{equation}
that maps outcomes a single value $\omega\in\Omega$ (the sample space) to
...
variable that numerically represents the coin flip,
$\Omega=\{\text{``Heads''},\text{``Tails''}\}$, and for example, if were
$\omega=\text{``Heads''}$ then $X(\omega)=0$, and viceversa, $X(\omega)=1$
if $\omega=\text{``Tails''}$.

Notice that, in general, a randon variable is not random itself (it's a
deterministic function of the underlying outcome: $X(\text{Heads})=0$,
always). The ``randomness'' comes from the fact that the outcome
$\omega$ is random (governed by the probability of resulting heads or
tails).

In the context of digital signals,  the samples can be considered as the output of a \gls{DRV}.

\section{\glsentryfull{DRVec}s} 

By definition, a \gls{DRV} can only model the generation of a
$x\in\mathcal{X}$ (in the context of digital signal, a \gls{DRV} would
handle a single sample). To deal with a complete digital signal (or
sequence or samples) we need to extend the concept of \gls{DRV} to
\gls{DRVec}.
