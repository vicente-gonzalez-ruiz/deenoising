...
values in some set $\mathcal{X}$ (usually $\mathbb{R}$, or a
countable/continuos subset). For example, if we toss a coin, $X$ is a
...
Notice that a randon variable is not random itself (it's a
deterministic function of the underlying outcome). The ``randomness''
comes from the fact that the outcome $\omega$ is random (governed by
the probability of resulting heads or tails).
