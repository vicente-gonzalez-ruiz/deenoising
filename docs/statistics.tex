...
\section{$n$-th moment of a random variable}
The $n$-th moment of a random variable $X$ is the expectation of the
$n$-th power of $X$:
\begin{equation}
  \mathbb{E}[X^n] = \int_\Omega(X(\omega))^nd\Pr(\omega).
\end{equation}

\section{The sample mean}
\label{sec:sample_mean}
The sample mean of a \gls{DRV} $X$, usually denoted by $\overline{x}$, is the
first moment of the realizations
\begin{equation}
  \overline{x} = \sum_\Omega X(\omega)d\Pr(\omega).
  \label{eq:sample_mean}
\end{equation}

\section{Average}
\label{sec:average}
The average of the realizations $x$ generated after $N$ iterations of
a random variable is defined by
\begin{equation}
  \overline{x} = \frac{1}{N}\sum_{i=1}^{N}x_i,
  \label{eq:average}
\end{equation}
where $x_i$ if the $i$-th realization.
