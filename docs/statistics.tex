...
\section{Univariate VS multivariate data}
%{{{
...
The \acrshort{CLT} states that when the realizations of a set $\bm{X}=\{\bm{X}_i\}_{i=0}^{N-1}$ of
\gls{iid} \glspl{DRV} (in general, \glspl{SRV}) (see
Section~\ref{sec:DRV}) are added together, the resulting sum is
another \gls{SRV} that tends toward a normal (Gaussian) distribution,
even if the original variables themselves are not normally
distributed:
\begin{equation}
  \overline{\bm{X}} = \frac{1}{N}\sum_{i=0}^{N-1}\bm{n}_i\sim\mathcal{N}(\mu,\frac{\sigma}{N}).
\end{equation}

%}}}

\section{\glsentryfull{GAT}} % Generalized Anscombe Transform
%{{{

The \acrshort{GAT} \cite{foi2008practical,makitalo2012optimal} is a
\gls{VST} that can be applied to realizations of a \gls{DRV} of
unknown distribution to make it approximately Gaussian with variance
$1$. It consists of converting the realizations $x$ using 
\begin{equation}
  y = \text{GAT}(x)=\frac{2}{\gamma}\sqrt{\gamma\hat{x}+\frac{3}{8}\gamma^2+\sigma^2},
\end{equation}
where now
\begin{equation}
...
Notice that the \gls{GAT} is not a linear transform and that the mean
is not preserved (the result is biased), in neither of the directions
of the transform. This basically implies that you cannot recover the
original mean by simply inversing the forward expression. For this
reason, in \cite{makitalo2012optimal} (among others) the following
approximation can be used only if we deal with positive transformed
values and $\mu=0$:
\begin{equation}
  x \approx \frac{1}{4}y^2-\frac{1}{4}\left(1+\frac{8\sigma^2}{y^2} + \frac{1}{32}\left(\frac{y+\frac{8\sigma^2}{y^2}}{y^2}\right)\right).
\end{equation}

The \gls{GAT} and previously related \glspl{VST} rely on the
compression of the dynamic-range of the values that can take the input
data. Thus, large input values are more compressed (where the variance
is larger) than small input values (where the variance is smaller).
