...
\begin{itemize}
\item For example, suppose we roll two fair six-sided dice. The sample
  space $\Omega$ consists of 36 ordered pairs $(d_1,d_2)$ where
  $d_i\in\{1,\cdots,6\}$. Now, we define a \gls{DRVec}
  $\mathcal{X}=[S,D]^{\text{T}}$, where $S=d_1 + d_2$ and
  $D=|d_1-d_2|$ are \glspl{DRV}. For example, if $\omega=(4,6)$, we
  get $\mathcal{X}(4,6)=[10,2]^{\text{T}}$. In this example, $N=2$.
\end{itemize}
