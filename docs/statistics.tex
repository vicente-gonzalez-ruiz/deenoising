...
\end{equation}
that maps outcomes $\omega\in\Omega$ (the sample space) to numerical
...
$\omega=\{\text{Heads},\text{Tails}\}$, by definition $X(\omega)=1$ if
$\omega=\text{Heads}$, and $X(\omega)=0$ if $\omega=\text{Tails}$.
