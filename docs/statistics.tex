...
\begin{equation}
  ...
  \label{eq:DRV}
\end{equation}
that maps outcomes a single random value $\omega\in\Omega$ (the sample space) to
a numerical value in some set $\mathcal{X}$, usually $\mathbb{N}$ or
a countable subset. For example, if we toss a coin, $X$ is a random
variable that numerically represents a coin flip $\omega''
($\Omega=\{\text{``Heads''},\text{``Tails''}\}$), and for example, if were
$\omega=\text{``Heads''}$ then $X(\omega)=0$, and viceversa, $X(\omega)=1$
if $\omega=\text{``Tails''}$.
