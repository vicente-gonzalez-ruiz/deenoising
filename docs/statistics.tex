...
\label{sec:average}
The \emph{sample mean} or \emph{arithmetic average} of the
realizations $\{x^{(i)}\}_{i=1}^{N}$ generated after $N$ observations
...
$N\rightarrow\infty$, the sample mean $\overline{x}$ converges to the
mean $\mu$. Therefore, the sample mean is an estimation of the mean
(the expected value) that should improve with $N$.
...
satisfied that
\begin{equation}
  \lim_{N\rightarrow\infity}\overline{x}
  \label{eq:LLN}
\end{equation}
...
\section{Variance}
\label{sec:variance}
The variance tells you how far the actual outcomes generated by a
random variable are likely to deviate from that expected value (mean). the variance of a random variable X is defined as the expected value of the squared deviation from the mean.
