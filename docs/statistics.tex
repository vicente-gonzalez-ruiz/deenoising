...
A random variable $X$ is a function that maps an \emph{outcome}
$\omega\in\Omega$ to a \emph{realization} $\mathcal{x}\in\mathcal{X}$:
\begin{equation}
  X:\Omega\rightarrow \mathcal{X}.
  \label{eq:DRV}
\end{equation}
$\Omega$ is known as the \emph{sample space} and $\mathcal{X}$ is
called the \emph{realization space}. Notice that $X$ is a
deterministic function. The randomness comes from the fact that the
underlying outcome is random. $\mathcal{X}$ must be also measurable
(otherwise it could be impossible to assign a probability
$P(X\leq x)$.

An example of a random variable is the The 10-Minute Bus
Scenario. Imagine you arrive at a bus stop where a bus comes exactly
every 10 minutes. You don't know when the last one left, so you could
wait anywhere from 0 to 10 minutes. In this example, the outcome
$\omega$ is the actual, physical event of you standing at the curb and
the bus eventually pulling up, and the sample space $\Omega$ is
continuous because you could arrive at any point in time between the
bus cycles. The random variable $X$ is ``how many minutes you wait'',
and $x\in\mathbb{R}$ is such number. Obviously $X$ measurable, because
we can compute, for example, $P(X\leq 5)$. This is an example of a
...
Example (continuous input): Let's consider again the 10-Minute Bus
Scenario. Here $\Omega=[0,10]$. Now, if we define
$\mathcal{X}=\{0,1\}$ as:
\begin{enumerate}
...
Example (discrete input): If we toss a coin, and define
$\mathcal{X}=\{0,1\}$ as:
\begin{enumerate}
...
\section{\glsentryfullpl{DRVec}} 

A \gls{DRVec} (also called a multivariate random variable) is a
generatization of a \gls{DRV} where $\mathcal{X}$ is
multi-dimensional. Let's $N$ the dimension of $\mathcal{X}$.

For example, suppose we roll two fair six-sided dice. The sample
space $\Omega$ consists of 36 ordered pairs $(d_1,d_2)$ where
$d_i\in\{1,\cdots,6\}$. Now, we define a \gls{DRVec}
$\mathcal{X}=[S,D]^{\text{T}}$, where $S=d_1 + d_2$ and $D=|d_1-d_2|$
are \glspl{DRV}. For example, if $\omega=(4,6)$, we get
$\mathcal{X}(4,6)=[10,2]^{\text{T}}$. In this example, $N=2$.

Notice that the vector representation expresses a dependency between
$S$ and $D$. For example, if you know hat $S=12$,
then the $D$ must be 0 (the dice must be 6 and 6). Such dependency
allows the definition of the Joint Probability Mass Function (PMF),
and the existence of the concepts covariance and correlation.

\section{A statistical model of signal digitalization}

The digitalization of a sequence of $N$ samples
can be modeled statistically using a \gls{DRVec}. We can recognize that:
\begin{itemize}
\item $\Omega$ (the sample space) is the set of all possible physical
  continuous-time waveforms that the sensor could encounter during the
  sampling window, and $\omega$ is a single, specific continuous
  voltage waveform that actually occurs during your measurement
  window. $\omega$ is the ``analog reality'' (the signal to
  digitalize).
\item $X$ represents the ADC (Analog-to-Digital Converter). It is a
  deterministic function that performs two operations on the outcome
  $\omega$:
  \begin{enumerate}
  \item Sampling: $X$ looks at $\omega$ at $N$ specific time points
    $\{t_1​,t_2​,\cdots,t_N\}$.
  \item Quantization: Then $X$ rounds the voltage at those points to
    the nearest integer (for example).
  \end{enumerate}
\item The realization $\mathbf{x}=X(\omega)$ is the digizalized
  signal. Notice that $X$ is a vector operator that inputs a sample (a
  window of signal) and outputs a sequence of digital samples
  $\{\mathbf{x}_1,\mathbf{x}_2,\cdots,\mathbf{x}_N\}$.
\item $\mathcal{X}$ (the realization space) is the set of all possible
...
Probability Mass Function (PMF), shows the probability for each
specific value that a \gls{DRV} can take.\footnote{A continuous random
variable is represented by a Probability Density Function (PDF).}

The \gls{PMF} of a random variable $X$, defined as
\begin{equation}
  p(x) = \Pr(X=x), \quad x\in\mathcal{X},
\end{equation}
describes the probability with which a random variable $X$ takes
values in a countable set $\mathcal{X}$, where it must be true that
\begin{equation*}
...
\section{Joint Probability Mass Function (PMF)}
A random vector variable allows us to define a single function $P(\mathbf{X}=\mathbf{x})$, which is shorthand for:
\begin{equation}
P(S = s \text{ and } D = d)
\end{equation}
This joint PMF tells us the probability of specific coordinates in the 2D space X. Without the vector framework, we could only describe the "marginal" probabilities (the probability of the sum regardless of the difference), losing the context of the experiment.
...
\section{Discrete signals and digital signals}
...
By physical requirements, digital signals are finite in length, and
therefore $\text{Sup}(\mathbf{x})$ is a finite set.

\section{Expectation of a \glspl{DRV}}

The expected value, expectation or expectancy of a random varaible is its the long-term average outcome.
The expected value (expectation, mathematical expectation,
or expectancy) of a \gls{DRV} $\mathbf{x}$, denoted as
$\mathbb{E}(\mathbf{x})$ (and also by $\mathbb{E}[\mathbf{x}]$), is a
theoretical concept representing the long-run average value of the
variable if we had an infinite number of samples. It is defined as the
weighted average of all possible values the random variable can take,
where the weights are the probabilities of those values occurring:
\begin{equation}
  \mathbb{E}(\mathbf{x}) = \sum_{\mathbf{x}_i}\mathbf{x}_i\Pr(\mathbf{x}=\mathbf{x}_i).
  \label{eq:expectation}
\end{equation}

For a digital signal $\mathbf{x}$, it is defined as
\begin{equation}
  \mathbb{E}(\mathbf{x})=\mathbb{E}_{\mathbf{x}}(\mathbf{x})=\sum_i\mathbf{x}_i\Pr(\mathbf{x}=\mathbf{x}_i).
  \label{eq:expectation_digital}
\end{equation}
...
Ed.~\ref{eq:expectation_digital} when all the samples have the same
probability.
...
its dispersion (how much the outcomes ``spread out'' from the
average). It is defined as the expected value of the squared deviation
from the mean,
%\begin{equation}
...
The covariance $\mathbb{C}(\mathbf{x}, \mathbf{y})$ (also denoted by
$R_{\mathbf{x}\mathbf{y}}$) is a measure of how two random variables
$\mathbf{x}$ and $\mathbf{y}$ change together. In simpler terms, it
tells us the direction of the linear relationship between two
variables. The covariance between two discrete signals $\mathbf{x}$
and $\mathbf{y}$ is calculated as
%\begin{equation}
