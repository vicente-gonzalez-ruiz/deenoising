...
\section{\glsentryfull{ARV}}
A \gls{DRV} is a function
\begin{equation}
  X:\Omega\rightarrow \mathcal{X}
  \label{eq:DRV}
\end{equation}
that maps outcomes $\omega\in\Omega$ (the sample space\footnote{That
in the context of digital data, also is the samples space.}) to
numerical values in some set $\mathcal{X}$, usually $\mathbb{N}^+$ or
...
A \gls{DRVec} expands the concept of a random variable by
mapping outcomes to a point in multi-dimensional space rather than a
single number. Now Eq.~\ref{eq:DRV} becomes
\begin{equation}
