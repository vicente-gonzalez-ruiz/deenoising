...
\begin{equation}
  \overline{\bm{x}} = \frac{1}{N}\sum_{i=1}^{N}x_i}.
  \label{eq:average}
\end{equation}
...
\section{\acrlong{LLN}} % Law Large Numbers
%{{{
\label{sec:LLN}
...
\begin{equation}
  ...
  \label{eq:variance}
\end{equation}
Notice that considering directly sequences of realizations, Equation~\ref{eq:variance} is equivalent to
\begin{equation}
  \text{Var}(\bm{x}) = \mathbb{E}[(\bm{x} - \mathbb{E}[\bm{x}])^2] = \mathbb{E}[\bm{x}^2]-\mathbb{E}[\bm{x}]^2.
  \label{eq:variance_x}
\end{equation}
