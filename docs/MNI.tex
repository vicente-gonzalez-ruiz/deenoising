...
pixel-wise averaging the noisy images. This technique is commonly
known as \emph{image stacking}. Doing that, in theory, it is spected
to increase the \gls{SNR} by a factor of $\sqrt{N}$, where $N$ is the
number of \emph{averaged}\footnote{Astronomers use the term
  ``stacked''.} images.
...
shot noise + interference) still ends up looking Gaussian.
