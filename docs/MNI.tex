\chapter{\glsentryfull{MNI}}
This chapter analyze how noise can be removed pixel-wise averaging the
noisy images. This technique is commonly known as \emph{image
  stacking}. Doing that, in theory, it is spected to increase the SNR
by a factor of $\sqrt{N}$, where $N$ is the number of stacked images.

\section{Applicability of the \gls{CLT}}
%{{{

The \gls{CLT} is applicable only if:
\begin{enumerate}
\item The mean of the noise is finite: $\mu<\infty$.
\item The variance of the noise is finite: $\sigma^2\infty$.
\end{enumerate}
Most of the natural noise sources satisfie these conditions. However,
notice that there are statistical distributions, such as the Cauchy
distribution that has infinite variance and an undefined mean. Thus,
the addition of a infinite number of sequences of random number that
follow (each of them) a Cauchy distribution is the Cauchy
distribution. This is the Lindeberg-Levy \gls{CLT}.

Fortunately, there exists a more relaxed ``version'' of th
Lindeberg-Levy \gls{CLT} known as the Lindeberg-Feller \gls{CLT} which
states that the \gls{CLT} works even if the random variables are not
\gls{iid}, provided that no single variable in the sequence dominates
the others. This is why the sum of many different types of noise (such
as thermal noise + shot noise + interference) still ends up looking
Gaussian.
...
The signal is the information you actually want to capture (in the
case of an image, the ``true'' brightness or density of your
sample). Therefore, using stacking, if you were to take the same
image 1000 times and average them together, the random fluctuations
would cancel each other out. What remains is the mean. In other words,
