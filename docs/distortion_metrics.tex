...
\section{Power \glsentryfull{SNR} (P-\gls{SNR})}
%{{{

The \acrshort{SNR} compares the energy of a clean signal (\gls{GT})
$\bm{X}$ to the energy of \emph{background} noise
$\bm{N}=\bm{X}-\tilde{\bm{s}}$. The ``power'' version of the \gls{SNR} is defined as the ratio of average signal
power to average noise power,
\begin{equation}
  \text{P-SNR}(\bm{X},\tilde{\bm{X}}) = \frac{P(\bm{X})}{P(\bm{X} - \tilde{\bm{X}})}.
  \label{eq:power_SNR}
\end{equation}
%where, in general, 
%\begin{equation}
...
of the power of the noise is also the variance of the noise.

In decibels, Eq.~\ref{eq:power_SNR} becomes
\begin{equation}
  \text{P-SNR}_{\text{dB}}(\bm{X},\tilde{\bm{X}}) = 10\log_{10}\text{P-SNR}(\bm{X},\tilde{\bm{X}}).
  \label{eq:power_SNR_dB}
\end{equation}
...
\section{Amplitude \glsentryfull{SNR} (A-\gls{SNR})}
%{{{

In the case of considering amplitudes (see
Section~\ref{sec:RMS_amplitude}) instead of power,
\begin{equation}
  \text{A-SNR}(\bm{X},\tilde{\bm{X}}) = \frac{\text{RMS}(\bm{X})}{\text{RMS}(\bm{X} - \tilde{\bm{X}})}.
  \label{eq:amplitude_SNR}
\end{equation}

In decibels,
\begin{equation}
  \text{A-SNR}_{\text{dB}}(\bm{X},\tilde{\bm{X}}) = 20\log_{10}\text{A-SNR}(\bm{X},\tilde{\bm{X}}).
  \label{eq:power_SNR}
\end{equation}

Notice if you double the power of a signal, your \gls{SNR} increases by
about 3dB. However, if you double the amplitude (voltage) of a signal,
you have actually quadrupled the power, so the \gls{SNR} increases by about
6dB.
...
\section{\glsentryfull{CoV}}
The \gls{CoV} is defined as the inverse of the \gls{SNR}:
\begin{equation}
  \text{CoV} = \frac{1}{\text{SNR}}.
\end{equation}
