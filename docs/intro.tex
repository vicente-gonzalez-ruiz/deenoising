...
\begin{enumerate}
  \item \textbf{\gls{LM}}: Light passes through the specimen, and this
    light is then magnified by an objective and ocular lenses to form
    an observable image. Fluorescence microscopy\footnote{The target
    of interest is ``tinted'' with fluorophores that upon ilumination
    with a specific wavelength of excitation light, the attached
    fluorophores emit photons.} and confocal microscopy are two
    \gls{LM} tecniques, where the specimen(s) emit light after have
    being excited by a source of light. Using \gls{LM} we can typical
    reach a resolution of up to 200 nm.
