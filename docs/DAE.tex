...
\section{Supervised (N2C) VS unsupervised denoising methods}
...
\section{Supervised VS zero-shot methods}
Historically, supervised approaches rely on large datasets with high-quality
labels. Zero-shot methods avoid this limitation but remain effective
only for \gls{iid} noise. % Median2Median: Zero-shot Suppression of Structured Noise in Images, J Wang, G Wang - arXiv preprint arXiv:2510.01666, 2025

This can be also done even in only one image is in the training dataset, using patching.
...
% A. Krull, T. Buchholz, and F. Jug, “Noise2void - learning denoising from single noisy images,” Proceedings of the International Conference on Computer Vision and Pattern Recognition (CVPR)2124–2132 (2019).
...
def cryoCARE-EO(noisy_vol):
  even_vol = noisy_vol[0::2,:,:] # Even Z-slices
  odd_vol = noisy_vol[1::2,:,:]  # Odd  Z-slices
  model = train(even_vols=[even_vol], odd_vols=[odd_vol])
  denoised_vol = predict(model, noisy_vol, noisy_vol)
  return denoised_vol

train(even_vols, odd_vols):
  model = cryoCARE.train(even=even_vols, odd=odd_vols)
  return model

predict(model, even_vols, odd_vols):
  denoised_vol = cryoCARE.predict(model, even=even_vols, odd=odd_vols)
  return denoised_vol
\end{verbatim}

\section{cryoCARE-REO  (cryoCARE Registered Even Odd)}
\begin{verbatim}
cryoCARE-REO(noisy_vol):
  even_vol = noisy_vol[0::2,:,:]
  odd_vol = noisy_vol[1::2,:,:]
  even_vol_registered = register_vol(even_vol, odd_vol)
    # even_vol_registered is an adaption of "even_vol" similar to "odd_vol".
  odd_vol_registered = register_vol(odd_vol, even_vol)
    # odd_vol_registered is an adaption of "odd_vol" similar to "even_vol".
  model = train(even_vols=[even_vol_registered, even_vol],
                 odd_vols=[odd_vol, odd_vol_registered])
    # The even-odd pairs in the lists must correspond to the same vol.
  denoised_vol = predict(model, noisy_vol, noisy_vol)
  return denoised_vol

register_vol(vol_to_warph, reference_vol):
  registered_vol = empty_like(reference_vol)
  for z in range(reference_vol.shape[0]):
...
\begin{figure}[t]
  \centering
  \href{https://github.com/vicente-gonzalez-ruiz/denoising/blob/main/docs/DAE/cryoCARE/empiar10311/denoise_even_odd.ipynb}{\includegraphics[width=1.0\textwidth]{~/gdrive_TomogramDenoising/deenoising/docs/DAE/cryoCARE/empiar10311_stack_crop/even_odd/denoised.pdf}}
...
\begin{figure}[t]
  \centering
  \href{https://github.com/vicente-gonzalez-ruiz/denoising/blob/main/docs/DAE/cryoCARE/empiar10311/denoise_odd_and_even_registered.ipynb}{\includegraphics[width=1.0\textwidth]{~/gdrive_TomogramDenoising/deenoising/docs/DAE/cryoCARE/empiar10311_stack_crop/even_odd_registered/denoised.pdf}}
  \caption{Denoising of EMPIAR-10311 (see Fig.~\ref{fig:empiar10311}) using
...
\begin{figure}[t]
  \centering
  \href{https://github.com/vicente-gonzalez-ruiz/denoising/blob/main/docs/DAE/cryoCARE/empiar11415/denoise_even_odd.ipynb}{\includegraphics[width=1.0\textwidth]{~/gdrive_TomogramDenoising/deenoising/docs/DAE/cryoCARE/empiar11415/even_odd/denoised.pdf}}
...
\begin{figure}[t]
  \centering
  \href{https://github.com/vicente-gonzalez-ruiz/denoising/blob/main/docs/DAE/cryoCARE/empiar11415/denoise_odd_and_even_registered.ipynb}{\includegraphics[width=1.0\textwidth]{~/gdrive_TomogramDenoising/deenoising/docs/DAE/cryoCARE/empiar11415/even_odd_registered/denoised.pdf}}
...
\begin{figure}[t]
  \centering
  \href{https://github.com/vicente-gonzalez-ruiz/denoising/blob/main/docs/DAE/cryoCARE/corbel_small/denoise_even_odd.ipynb}{\includegraphics[width=1.0\textwidth]{~/gdrive_TomogramDenoising/deenoising/docs/DAE/cryoCARE/Corbel2301_block2_June2019_crop_ali_crop/even_odd/denoised.pdf}}
  \caption{Denoising of Corbel Small (see Fig.~\ref{fig:corbel_small}) using
    cryoCARE-EO.\label{fig:cryoCARE_corbel_small_denoise_even_odd}}
\end{figure}

\begin{figure}[t]
  \centering
  \href{https://github.com/vicente-gonzalez-ruiz/denoising/blob/main/docs/DAE/cryoCARE/corbel_small/denoise_odd_and_even_registered.ipynb}{\includegraphics[width=1.0\textwidth]{~/gdrive_TomogramDenoising/deenoising/docs/DAE/cryoCARE/Corbel2301_block2_June2019_crop_ali_crop/even_odd_registered/denoised.pdf}}
  \caption{Denoising of Corbel Small using  (see Fig.~\ref{fig:corbel_small})
    cryoCARE-REO.\label{fig:cryoCARE_corbel_small_denoise_even_odd_registered}}
\end{figure}

\begin{figure}[t]
  \centering
  \href{https://github.com/vicente-gonzalez-ruiz/denoising/blob/main/docs/DAE/cryoCARE/epfl1/denoise_even_odd.ipynb}{\includegraphics[width=1.0\textwidth]{~/gdrive_TomogramDenoising/deenoising/docs/DAE/cryoCARE/epfl1_subset1_float/even_odd/denoised.pdf}}
  \caption{Denoising of EPFL 1 Subset 1 (see
    Fig.~\ref{fig:epfl1_subset1_float}) using
    cryoCARE-EO.\label{fig:cryoCARE_epfl1_subset1_float_denoise_even_odd}}
\end{figure}

\begin{figure}[t]
  \centering
  \href{https://github.com/vicente-gonzalez-ruiz/denoising/blob/main/docs/DAE/cryoCARE/epfl1/denoise_odd_and_even_registered.ipynb}{\includegraphics[width=1.0\textwidth]{~/gdrive_TomogramDenoising/deenoising/docs/DAE/cryoCARE/epfl1_subset1_float/even_odd_registered/denoised.pdf}}
  \caption{Denoising of EPFL1 Subset 1 using  (see Fig.~\ref{fig:epfl1_subset1_float})
    cryoCARE-REO.\label{fig:cryoCARE_epfl1_subset1_float_denoise_even_odd_registered}}
\end{figure}
