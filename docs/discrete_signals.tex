...
bold-faced symbols, such as $\mathbf{x}$. By physical reasons, we will
suppose that discrete signals are finite. A discrete signal
$\mathbf{x}$ will be denoted as
\begin{equation}
  \mathbf{x} = \{\mathbf{x}_n\}_{n=0}^{N-1},
\end{equation}
where $N$ is the signal length, and $\mathbf{x}_n$ is the $n$-th
(digital-)sample of $\mathbf{x}$.
...
Discrete signal are sequences of samples. For example, if $x(t)$ is a
analog signal that depends on time, the $n$-th signal analog-sample is
defined by
\begin{equation}
