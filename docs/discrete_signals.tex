...
(signal) samples. By physical reasons, we will suppose that discrete
signals are finite. A discrete signal $\mathbf{s}$ will be
denoted\footnote{Discrete signals will be represented with lower-case
bold-faced symbols, such as $\mathbf{s}$.} as
\begin{equation}
...
example, if $s(t)$ is an analog signal that depends on time, the $n$-th
...
The energy of a discrete signal $\mathbf{s}$ is the sum of the square
of all its samples:
\begin{equation}
...
The \acrshort{DFT} is the Fourier transform of a discrete finite signal $\mathbf{s}$ is defined as
\begin{equation}
...
\emph{phase spectrum} of $\mathbf{S}$ using
\begin{equation}
...
The \gls{PS} of finite-length discrete signal $\mathbf{s}$ shows how
much power (see Section~\ref{sec:power_signal}) a signal has at its
different frequency bins. Let $\mathbf{S}$ the Fourier transform of
$\mathbf{s}$. Then,
\begin{equation}
...
For two signals $\mathbf{s}$ (the signal to filter) and $\mathbf{h}$
(the filter coefficients or weights\footnote{In the rest of this
document, we will use only the term ``weight'' for refering to the
``weights of a filter'' and the term ``coefficien'' for refering to
the ``coefficients of a transform'' (the output of the transform).}),
...
Laplace transform of the input, and $s=\sigma+j\omega$ represents a complex
