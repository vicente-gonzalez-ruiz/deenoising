...
An (analog) signal is the result of the measurement of some physical
...
suppose that discrete signals are finite.}, and $\mathbf{x}_n$ is the
$n$-th (digital-)sample of $\mathbf{x}$, is a sequence of
samples. Continuing with the SPL example, each sample is found with
\begin{equation}
...
Discrete signal are sequences of (analog) samples. For example, if
$x(t)$ is a analog signal that depends on time, the $n$-th signal
analog-sample is defined by
\begin{equation}
  \mathbf{x}_n = x(t)\delta(t-nT),
\end{equation}
where  $T$ is the sampling period, and
...
bold-faced symbols, such as $\mathbf{x}$. A discrete signal
$\mathbf{x}$ will be denoted as
\begin{equation}
  \mathbf{x} = \{\mathbf{x}_n\}_{n=0}^{N-1},
\end{equation}
where $N$ is the signal length\footnote{By physical reasons, we will
suppose that discrete signals are finite.}, and $\mathbf{x}_n$ is the
$n$-th (digital-)sample of $\mathbf{x}$.

%However, notice that the assumption of this constrain ($N$ finite)
%only obey to the idea of focusing our analysis into the
%\emph{expected} case. In other words, a similar
...
The \acrshort{DFT} is the Fourier transform of a discrete finite signal $\mathbf{x}$ is defined as
\begin{equation}
