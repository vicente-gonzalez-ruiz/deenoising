...
A signal $s$ is the result of the measurement of some physical
event. For example, if $x$ represents a SPL (Sound Preasure Level),
$x(t)$ is the SPL at the instant of time $t$.
...
The \gls{DFT} of a (by definition, discrete and finite) signal
$\bm{x}$ is determined by
\begin{equation}
  \mathcal{F}(\bm{x}) = \mathbf{X} = \{\mathbf{X}_f\}_{f=0}^{N-1} \quad \text{with} \quad \mathbf{X}_f=\sum_{n=0}^{N-1}\mathbf{x}_ne^{-2\pi jf\frac{n}{N}},
  \label{eq:DFT}
\end{equation}
where $f$ denotes (discrete) frequency bins. The inverse \gls{DFT} is defined by
\begin{equation}
  \bm{x} = \mathcal{F}^{-1}(\mathbf{X}) \quad \text{with} \quad \bm{x}_n = \frac{1}{N}\sum_{f=0}^{N-1}\mathbf{X}_fe^{2\pi jn\frac{f}{N}}.
\end{equation}
...
discrete (and finte) signal $\bm{x}$, the Parseval’s theorem
states that
\begin{equation}
...
(or frequency) spectrum of $\bm{x}$ is the magnitude of
$\mathbf{X}$, that is
\begin{equation}
...
The \gls{PS} of finite-length discrete signal $\bm{x}$ shows how
...
$\bm{x}$. Then,
\begin{equation}
  \text{PS}(\bm{x}) = \{\text{PS}(\bm{x})_f\}_{f=0}^{N-1} \quad \text{where} \quad \text{PS}(\bm{x})_f = \frac{1}{N}|\mathbf{X}_f|^2,
\end{equation}
...
another signal (usually, the response of the system to an
impulse). An example of such system can be a filter.

For two signals $\bm{x}$ (the signal to be filtered) and
$\bm{h}$ (the filter weights), the convolution is defined as
\begin{equation}
(\bm{x}\ast\bm{h})_{n}=\sum_{k=0}^{K-1}\mathbf{x}_{n-k}\mathbf{h}_{k},
\label{eq:convolution}
\end{equation}
where $K$ is the number of weights, and $n$ is known as the \emph{lag}
(where the filter is applied in $\bm{x}$).

Notice that we could need to extend $\bm{x}$ to convolve it with
$\bm{h}$.
...
function of a lag $n\in\{n\}_{n=1}^{N}$. The \gls{CC} is
defined as
\begin{equation}
  (\mathbf{x}\circledast\mathbf{h})_n=\sum_{k=1}^{N}{\mathbf{x}}_{n+k} \mathbf{h}_n.
...
\section{\acrlong{AC}} % Auto-correlation
%{{{
\label{sec:auto-correlation}

The \gls{AC} measures the similarity (through a \gls{CC} operation)
