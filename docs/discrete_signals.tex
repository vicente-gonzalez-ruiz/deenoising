...
For two signals $\mathbf{x}$ (the signal to filter) and
...
\end{equation}
where $K$ is the number of weights (of the filter), and $n$ is
known as the \emph{lag} (where the filter is applied in $\mathbf{x}$).

Notice that we could need to extend $\mathbf{x}$ to convolve.
